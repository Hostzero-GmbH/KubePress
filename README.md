...
### Create the `kubepress-controller` namespace

We need a dedicated namespace for the KubePress controller. We use `kuberess-controller` as the namespace name, but any other name should work as well.

```bash
kubectl create namespace kubepress-controller
```

This needs to be done beforehand, because we need the docker registry credentials in the `kubepress-controller` namespace.

### Create some registry credentials in the `kubepress-controller` namespace

For Gitlab Container Registry you can create a deployment token under settings -> repository -> deploy tokens.

```bash
kubectl create secret docker-registry registry-credentials \
  --docker-server=registry.threerabbits.io \
...
```bash
helm upgrade --install kubepress dist/chart/ --namespace kubepress-controller -f dist/chart/values.yaml
```
